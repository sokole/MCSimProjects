...
design_matrix <- expand.grid(
  ...
  my_niche_breadth = c(0.3, 20), #filtering strong enough for tradeoffs -- subsurface has filter selecting for "resistant" taxa, surface selects for both equally
  
  # disturbance characteristics
  my_surface_mortality_percent = c(0.05, 0.50, 0.95),
  my_subsurface_mortality_percent = 0,
  
  my_subsurface_m_immigration_disturbance = .05, #does not change
  my_surface_m_immigration_disturbance = .9,
  
  disturbance_extent = c(0.05, 0.50, 0.95),
  disturbance_duration = 3,
  
  # recovery_duration <- 6
  recovery_duration = c(6, 25, 50),
  sim_rep = c(1:n_sim_reps))
#########################################

i_row_design_matrix <- 1

for(i_row_design_matrix in 1:nrow(design_matrix)){
  
  try({
    
    design_matrix_i <- design_matrix[i_row_design_matrix,] 
    
    attach(design_matrix_i) 
    ...
    my_sim_result <- MCSim::metasim.disturbance(
      ...
      # saving output
      save.sim = FALSE,
      output.dir.path = sim_result_dir_name)
    
    #########
    # -- save my_sim_result
    
    my_save_path <- paste0(sim_result_dir_name,'/SIM_RESULT',
                           my_sim_result$scenario.ID,
                           '.RDA')
    save(my_sim_result, file = my_save_path)
    
    detach(design_matrix_i) 
    print(paste('COMPLETED',i_row_design_matrix,'out of', nrow(design_matrix), 'sims'))
